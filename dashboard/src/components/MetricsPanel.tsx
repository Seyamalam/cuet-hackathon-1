  ...
  const fetchMetrics = useCallback(async () => {
    ...
    try {
      ...
      const [
        ...
        queryPrometheus(
          prometheusUrl,
          'sum(http_requests_total{status=~"4..|5.."}) or vector(0)',
        ),
      ...
      const errorRate =
        totalReqValue > 0 ? (errorCount / totalReqValue) * 100 : 0;
      ...
      setMetrics([
        {
          name: "Total Requests",
          value: totalReqValue.toLocaleString(),
          unit: "",
          icon: TrendingUp,
          color: "text-blue-400",
        ...
        {
          name: "Avg Latency",
          value: isNaN(latencyValue) ? "0" : (latencyValue * 1000).toFixed(1),
          unit: "ms",
          icon: Clock,
        ...
        {
          name: "P95 Latency",
          value: isNaN(p95Value) ? "0" : (p95Value * 1000).toFixed(1),
          unit: "ms",
          icon: Gauge,
        ...
        {
          name: "P99 Latency",
          value: isNaN(p99Value) ? "0" : (p99Value * 1000).toFixed(1),
          unit: "ms",
          icon: Gauge,
          color: "text-red-400",
        ...
        },
        {
          ...
          unit: "%",
          icon: AlertTriangle,
          color: errorRate > 5 ? "text-red-400" : "text-green-400",
          bgColor: errorRate > 5 ? "bg-red-500/10" : "bg-green-500/10",
        },
        {
          name: "Active Downloads",
          value: activeDown[0]?.value[1] || "0",
          unit: "",
          icon: Download,
        ...
        {
          name: "S3 Operations",
          value: parseInt(s3Ops[0]?.value[1] || "0").toLocaleString(),
          unit: "",
          icon: Database,
          color: "text-cyan-400",
      ...
      // Endpoint breakdown
      const endpointData = await queryPrometheus(
        prometheusUrl,
        "sum by (path, method) (http_requests_total)",
      );
      const endpointLatency = await queryPrometheus(
        prometheusUrl,
      ...
      // System metrics
      const [cpuUsage, memUsage, heapUsed, heapTotal, eventLoopLag] =
        await Promise.all([
          queryPrometheus(
            prometheusUrl,
      ...
      setSystemMetrics([
        {
          name: "CPU Usage",
          value: parseFloat(cpuUsage[0]?.value[1] || "0").toFixed(1),
          unit: "%",
          icon: Cpu,
          color: "text-blue-400",
          bgColor: "bg-blue-500/10",
        ...
        {
          name: "Memory",
          value: parseFloat(memUsage[0]?.value[1] || "0").toFixed(0),
          unit: "MB",
          icon: HardDrive,
          color: "text-purple-400",
          bgColor: "bg-purple-500/10",
        ...
        {
          name: "Heap Used",
          value: parseFloat(heapUsed[0]?.value[1] || "0").toFixed(0),
          unit: "MB",
          icon: Database,
          color: "text-cyan-400",
          bgColor: "bg-cyan-500/10",
        ...
        {
          name: "Heap Total",
          value: parseFloat(heapTotal[0]?.value[1] || "0").toFixed(0),
          unit: "MB",
          icon: Database,
        ...
        {
          name: "Event Loop Lag",
          value: parseFloat(eventLoopLag[0]?.value[1] || "0").toFixed(2),
          unit: "ms",
          icon: Activity,
          color: "text-yellow-400",
          bgColor: "bg-yellow-500/10",
    ...
    <div className="bg-slate-800 rounded-lg p-6 border border-slate-700">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-3 mb-6">
        {metrics.map((metric) => (
          <div
            key={metric.name}
            className={`${metric.bgColor} rounded-lg p-3 border border-slate-700/50`}
          >
                ...
                >
                  ...
                  <div className="flex items-center gap-4 text-xs">
                    <span className="text-slate-400">
                      {ep.requests.toLocaleString()} req
                    </span>
      ...
      <div className="mt-4 grid grid-cols-2 md:grid-cols-4 gap-2">
        <a
          href={`${prometheusUrl}/graph?g0.expr=up&g0.tab=0`}
          target="_blank"
          rel="noopener noreferrer"
          className="bg-slate-900/50 hover:bg-slate-700/50 rounded-lg p-3 flex items-center gap-2 text-sm transition-colors"
        >
          <span className="w-2 h-2 rounded-full bg-green-400"></span>
          Service Health
        </a>
        <a
          href={`${prometheusUrl}/graph?g0.expr=rate(http_requests_total[5m])&g0.tab=0`}
          target="_blank"
          rel="noopener noreferrer"
          className="bg-slate-900/50 hover:bg-slate-700/50 rounded-lg p-3 flex items-center gap-2 text-sm transition-colors"
        >
        ...
        <a
          href={`${prometheusUrl}/graph?g0.expr=histogram_quantile(0.95,%20sum(rate(http_request_duration_seconds_bucket[5m]))%20by%20(le))&g0.tab=0`}
          target="_blank"
          rel="noopener noreferrer"
          className="bg-slate-900/50 hover:bg-slate-700/50 rounded-lg p-3 flex items-center gap-2 text-sm transition-colors"
        >
        ...
        <a
          href={`${grafanaUrl}/d/delineate/delineate-dashboard`}
          target="_blank"
          rel="noopener noreferrer"
          className="bg-slate-900/50 hover:bg-slate-700/50 rounded-lg p-3 flex items-center gap-2 text-sm transition-colors"
        >
